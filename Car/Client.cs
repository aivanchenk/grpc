	...
	private void Run() {
		...
		while( true )
		{
			try {
				...
				var trafficLight = new TrafficLight.TrafficLightClient(channel);
				...
				car.CarId = trafficLight.GetUniqueId(new Empty()).Value;
				...
				while( true )
				{
					...
					while( !isCrashed && !isPassed )
					{
						...
						var lightState = trafficLight.GetLightState(new Empty()).Value;
						...
						else
						{
							...
							var inQueue = trafficLight.Queue(car).Value;
							...
							if( inQueue )
							{
								...
								while( !isCrashed && !isPassed )
								{
									...
									lightState = trafficLight.GetLightState(new Empty()).Value;
									var firstInLine = trafficLight.IsFirstInLine(new IntMsg { Value = car.CarId}).Value;
